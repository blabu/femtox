...
static const unsigned short TIMER_CONST = 40;
...
unsigned short _setTickTime(unsigned short timerTicks) {
  unsigned short oldTimer = TIMERB_10MS;
  TIMERB_10MS = TIMER_CONST;
  unsigned short i = 1;
  for(; i<timerTicks; i++) {
    TIMERB_10MS += TIMER_CONST;
    if(TIMERB_10MS > 0xFFFE - (TIMER_CONST<<1)) break; // Следующая итерация может переполнить счетчик
  }
